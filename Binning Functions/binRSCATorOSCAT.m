function bin=binRSCATorOSCAT(fileNames,dLat,startLat,endLat,...
    dLon,startLon,endLon,NtPerDay,dateCell,inputDataSource,binArgs)
% plot_wind_dir.m ----------------------------------------------------------
% 
...
if nargin<11
    ...
    binArgs.dataSource=inputDataSource;
end
...
% IMPORTANT: Ignore time value convention from product guides! Below comes
% straight from the netcdf files themselves!
rapidscatStart=[1999 01 01 0 0 0];
% Read in MJO indices
...
fprintf(['Test MJO: ' num2str(binArgs.mjo) '\n']);
fprintf(['MJO phases ' MJOstring '.\n']);
...
for i=1:numIntervals
    ...
    for k=1:length(fileNames{i})
        ...
        netCDF.time=repmat(netCDF.time,size(netCDF.lat,1),1);
        netCDF.wind_speed=ncread(fileNames{i}{k},'retrieved_wind_speed');
        netCDF.wind_dir=ncread(fileNames{i}{k},'retrieved_wind_direction');
        ...
        netCDF.date=datevec(datetime(rapidscatStart)+seconds(min(netCDF.time(:))));
        netCDF.dateOnly=netCDF.date;
        netCDF.dateOnly(4:6)=[0 0 0];

        if binArgs.mjo
            % Specify MJO index from BOM data
            netCDF.mjoPhase=MJOdata(etime(netCDF.dateOnly,[1990 1 1 0 0 0])./(24*60*60)+1,4);
            netCDF.mjoAmp=MJOdata(etime(netCDF.dateOnly,[1990 1 1 0 0 0])./(24*60*60)+1,5);
        ...
        startTimeSec=etime(startTime,rapidscatStart);
        endTimeSec=etime(endTime,rapidscatStart);
        ...
        kT=floor((etime(rapidscatStart,startTime)+netCDF.time)/bin.dt)+1;
