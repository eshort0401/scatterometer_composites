function bin=binWRF(fileNames,dLat,startLat,endLat,...
...
for i=1:numIntervals
    ...
    for k=1:length(fileNames{i})
        ...
        year=str2double(fileNames{i}{k}(length(fileNames{i}{k})-21:length(fileNames{i}{k})-18));
        month=str2double(fileNames{i}{k}(length(fileNames{i}{k})-16:length(fileNames{i}{k})-15));
        day=str2double(fileNames{i}{k}(length(fileNames{i}{k})-13:length(fileNames{i}{k})-12));
        ...
        netCDF.u=squeeze(ncread(fileNames{i}{k},'U',[1 1 1 1], [Inf Inf 1 Inf], [1 1 1 1]));
        netCDF.v=squeeze(ncread(fileNames{i}{k},'V',[1 1 1 1], [Inf Inf 1 Inf], [1 1 1 1]));

        % De-stagger u and v
        netCDF.u=(netCDF.u(1:size(netCDF.u,1)-1,:,:)+netCDF.u(2:size(netCDF.u,1),:,:))./2;
        netCDF.v=(netCDF.v(:,1:size(netCDF.v,2)-1,:)+netCDF.v(:,2:size(netCDF.v,2),:))./2;

        % These temp arrays will store the averaged lat and lon data.
        uSumLat=zeros(size(netCDF.lat,1),length(bin.y),24);
        tSumLat=zeros(size(netCDF.lat,1),length(bin.y),24);
        vSumLat=zeros(size(netCDF.lat,1),length(bin.y),24);
        countLat=zeros(size(netCDF.lat,1),length(bin.y),24);

        % Form time array
        netCDF.time=ones(size(netCDF.u,1),size(netCDF.u,2),24);
        for n=1:24
            netCDF.time(:,:,n)=(n-1)*ones(size(netCDF.u,1),size(netCDF.u,2))*60*60;
        end
        ...
        for m=1:length(bin.y)
            uSumLat(:,m,:)=sum(netCDF.u(:,kLat==m,:,:),2);
            vSumLat(:,m,:)=sum(netCDF.v(:,kLat==m,:,:),2);
        ...
        for m=1:length(iInd) % Iterate over swath and time.
            ...
            uI(kLon(iInd(m),kInd(m)),:,kT(iInd(m),kInd(m)))=...
                uI(kLon(iInd(m),kInd(m)),:,kT(iInd(m),kInd(m)))+uSumLat(iInd(m),:,kInd(m));
            vI(kLon(iInd(m),kInd(m)),:,kT(iInd(m),kInd(m)))=...
                vI(kLon(iInd(m),kInd(m)),:,kT(iInd(m),kInd(m)))+vSumLat(iInd(m),:,kInd(m));
...
% satellite track.
if ~isnan(ASCATcount)
    bin.u(ASCATcount==1)=0;
    bin.v(ASCATcount==1)=0;
    bin.count(ASCATcount==1)=0;
    bin.time(ASCATcount==1)=0;
end

bin.u=bin.u./bin.count;
bin.v=bin.v./bin.count;
bin.time=bin.time./bin.count;

bin.u(isnan(bin.u) | isinf(bin.u))=0;
bin.v(isnan(bin.v) | isinf(bin.v))=0;
